...
def hisEqulColor(img):
	...
	if(np.mean(channels[0] ) < 127):
		cv2.equalizeHist(channels[0],channels[0])
		cv2.merge(channels,yuv)
		cv2.cvtColor(yuv,cv2.COLOR_YUV2BGR,img)
...
def main():
	...
	while True:
		...
		#cv2.putText(frame,"is doing",(100,100),cv2.FONT_HERSHEY_COMPLEX, 1, (255,255,255))
		...
		for i in range(len(result[0][0])):
			...
			if confidence > 0.5:
				...
				#cv2.putText(frame, str(i), (50,50),cv2.FONT_HERSHEY_COMPLEX, 1, (255,255,255))
				if xmin<0:
					xmin = 0
				if ymin<0:
					ymin = 0
		...
		# cv2.putText(frame,"yaw: {}".format(str(yaw)), (20,30),cv2.FONT_HERSHEY_COMPLEX, 1, color,1)
		# cv2.putText(frame,"pitch: {}".format(str(pitch)), (20,60),cv2.FONT_HERSHEY_COMPLEX, 1, color,1)
		# cv2.putText(frame,"roll: {}".format(str(roll)), (20,90),cv2.FONT_HERSHEY_COMPLEX, 1, color,1)
		...
		crop_dlib = cv2.resize(crop, (300,300))
		...
		for (i, rect) in enumerate(rects):
			...
			if(ear_l < m_EAR_left and ear_r < m_EAR_right):
				cv2.putText(frame, "closed", (100,80),cv2.FONT_HERSHEY_COMPLEX, 1, (255,255,255))
				...
				cv2.putText(frame, "open", (100,80),cv2.FONT_HERSHEY_COMPLEX, 1, (255,255,255))
